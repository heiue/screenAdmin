        ...
        <script>
            layui.use(['layer','table','form'],function () {
                ...
                var dataTable = table.render({
                    elem: '#dataTable'
                    ,height: 500
                    ,url: "{{ route('admin.elite.data') }}" //数据接口
                    ,page: true //开启分页
                    ,cols: [[ //表头
                ...
                table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'del'){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.elite.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                                if (result.code==0){
                        ...
                        location.href = '/admin/knowledge/elite/'+data.id+'/edit';
                    }
                });
                ...
                $("#listDelete").click(function () {
                    ...
                    if (ids.length>0){
                        layer.confirm('确认删除吗？', function(index){
                            $.post("{{ route('admin.elite.destroy') }}",{_method:'delete',ids:ids},function (result) {
                                if (result.code==0){
                                    dataTable.reload()
                                }
                                layer.close(index);
